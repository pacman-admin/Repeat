...
public class TaskGroup implements IJsonable {
    ...
    public static TaskGroup parseJSON(DynamicCompilerManager factory, JsonNode node, ConfigParsingMode parseMode) {
        try {
            ...
            for (JsonNode task : node.getArrayNode("tasks")) {
                //sourceFilePath = task.getStringValue("source_path");
    ...
    public JsonRootNode jsonize() {
        ...
        return JsonNodeFactories.object(JsonNodeFactories.field("group_id", JsonNodeFactories.string(groupId)), JsonNodeFactories.field("name", JsonNodeFactories.string(name)), JsonNodeFactories.field("enabled", JsonNodeFactories.booleanNode(enabled)), JsonNodeFactories.field("tasks", JsonNodeFactories.array(taskNodes)));
    }
}