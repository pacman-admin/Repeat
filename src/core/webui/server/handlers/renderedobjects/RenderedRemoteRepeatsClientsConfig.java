...
public class RenderedRemoteRepeatsClientsConfig {
    ...
    public static RenderedRemoteRepeatsClientsConfig of(RepeatsPeerServiceClientManager manager, AbstractRemoteRepeatsClientsConfig config) {
        Set<String> enabledClients = new HashSet<>(config.getClients());

        List<RenderedConfigRemotePeerClient> clients = Stream.concat(Stream.concat(config.getClients().stream(), Stream.of(AbstractRemoteRepeatsClientsConfig.LOCAL_CLIENT)), manager.getClients().stream().map(RepeatsPeerServiceClient::getClientId)).distinct().sorted((String o1, String o2) -> {
        ...
        }).map(c -> RenderedConfigRemotePeerClient.of(manager, c, enabledClients.contains(c))).collect(Collectors.toList());
