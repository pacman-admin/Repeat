...
public final class TaskCliActionHandler extends CliActionProcessor {
    ...
    public void handle(Namespace namespace) {
        ...
        switch (action) {
            case "add" -> handleAdd(namespace);
            case "remove" -> handleRemove(namespace);
            case "execute" -> handleExecute(namespace);
            case "ls", "list" -> handleList(namespace);
            default -> {
