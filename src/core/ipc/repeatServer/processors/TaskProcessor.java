...
public final class TaskProcessor extends AbstractMessageProcessor {
    ...
    public boolean process(String type, long id, JsonNode content) throws InterruptedException {
        return ApiProtocol.isReplyMessage(content) ? processReply(id, content) : taskRequestProcessor.process(type, id, content);

    }

    private boolean processReply(long id, JsonNode content) {
    ...
    public String createTask(File file) {
        JsonRootNode requestMessage = JsonNodeFactories.object(JsonNodeFactories.field("task_action", JsonNodeFactories.string(CREATE_TASK_ACTION)), JsonNodeFactories.field("parameters", JsonNodeFactories.array(JsonNodeFactories.string(file.getAbsolutePath()))));
