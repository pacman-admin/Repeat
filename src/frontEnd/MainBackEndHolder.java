...
@SuppressWarnings({"unused", "DanglingJavadoc", "ResultOfMethodCallIgnored"})
public final class MainBackEndHolder {
    ...
    void loadConfig(File file) {
        ...
        File pythonExecutable = (config.getCompilerFactory()).getNativeCompiler(Language.PYTHON).getPath();
    ...
    public String generateSource() {
        return applySpeedup() ? recorder.getGeneratedCode(compilingLanguage) : "";
    }
    ...
    public boolean compileSourceAndSetCurrent(String source, String taskName) {
        ...
        RemoteRepeatsDyanmicCompilationResult remoteCompilationResult = remoteRepeatsCompiler.compile(source, compilingLanguage);
    ...
    public void setToolsClients(List<String> clients) {
        ...
        List<ITools> tools = clients.stream().map(c -> c.equals(AbstractRemoteRepeatsClientsConfig.LOCAL_CLIENT) ? Tools.local() : new RemoteRepeatsClientTools(peerServiceClientManager, c)).collect(Collectors.toList());
    ...
    public int getCurentTaskGroupIndex() {
        TaskGroup current = currentGroup;
